...
Options:
  ...
  -r <reference genome>   (REQUIRED) Path to reference genome file (fasta).
...
while getopts ":s:f:r:o:n:h" opt; do
  case "$opt" in
    s) SAMPLE="$OPTARG" ;;
...
# Basic validation
if [[ ! ${SAMPLE+x} || ! ${FASTQ_FILES_DIR+x} || ! ${REF_GENOME+x} || ! ${OUTPUTS_DIR+x} ]]; then
